...
#region Variables - Define NSG names for each region
Write-Host "=== Multi-Region NSG Association Script ===" -ForegroundColor Cyan
Write-Host "Started at: $(Get-Date)" -ForegroundColor Gray

# Define NSG names for each region (all in same resource group)
$RegionalNSGs = @{
    "East US" = "nsg-eastus-default"
    "West US" = "nsg-westus-default"
    "North Europe" = "nsg-northeurope-default"
    "Southeast Asia" = "nsg-southeastasia-default"
}

Write-Host "NSG Resource Group: $NSGResourceGroupName" -ForegroundColor Yellow
Write-Host "Configured NSGs for regions:" -ForegroundColor Yellow
foreach ($region in $RegionalNSGs.Keys) {
    Write-Host "  $region -> $($RegionalNSGs[$region])" -ForegroundColor White
}
#endregion
...
function Get-RegionalNSG {
    ...
    if ($RegionalNSGs.ContainsKey($Region)) {
        try {
            $nsgName = $RegionalNSGs[$Region]
            $nsg = Get-AzNetworkSecurityGroup -ResourceGroupName $NSGResourceGroupName -Name $nsgName -ErrorAction Stop
            return $nsg
        }
        catch {
            Write-ColorOutput "  ✗ Failed to get NSG '$nsgName' in resource group '$NSGResourceGroupName': $($_.Exception.Message)" "Red"
            return $null
        }
    }
    else {
...
function Invoke-RegionalNSGAssociation {
    ...
    foreach ($regionGroup in $vnetsByRegion) {
        ...
        $regionalNSG = Get-RegionalNSG -Region $region -NSGResourceGroupName $NSGResourceGroupName -RegionalNSGs $RegionalNSGs
...
try {
    ...
    Write-ColorOutput "NSG Resource Group: $NSGResourceGroupName" "White"
    ...
    foreach ($region in $RegionalNSGs.Keys) {
        ...
        try {
            $nsg = Get-AzNetworkSecurityGroup -ResourceGroupName $NSGResourceGroupName -Name $nsgName -ErrorAction Stop
            Write-ColorOutput "✓ NSG '$nsgName' found for region '$region'" "Green"
        }
        catch {
            Write-ColorOutput "✗ NSG '$nsgName' not found for region '$region': $($_.Exception.Message)" "Red"
    ...
    $statistics = Invoke-RegionalNSGAssociation -RegionalNSGs $RegionalNSGs -NSGResourceGroupName $NSGResourceGroupName -ExcludeSubnets $ExcludeSubnets -WhatIfMode $WhatIf -ForceMode $Force
